...
class U21ModeFlaskTests(unittest.TestCase):
    ...
    def test_pbp_result_mode_uses_pbp_result_loader(self):
        ...
        self.assertIn(b"https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ?autoplay=1&mute=0&playsinline=1&rel=0", response.data)
        self.assertIn(b"https://www.youtube.com/watch?v=dQw4w9WgXcQ", response.data)
        self.assertIn(b"Open video on YouTube", response.data)
        self.assertIn(b"Click to watch video and see results", response.data)
        self.assertIn(b'id="pbpResultContent" hidden', response.data)
        self.assertIn(b'id="confirmPbpResultBtn" hidden disabled', response.data)
